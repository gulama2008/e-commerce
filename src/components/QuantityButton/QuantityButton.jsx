...
const QuantityButton = ({ item, index, setQuantityInCart, quantityInCart }) => {
  const { products, changeItemQuantityInCart } = useContext(ProductsContext);
  ...
  const increaseQuantity = () => {
    const itemToBeDeductFromStock = products.find(
  ...
  return (
    <div className={styles.container}>
      ...
      {quantity == item.stock && (
        <p className={styles.warning}>You reach the maximum number of stock!</p>
      )}
    </div>
  );
};
